...
def test_prepare_review_sends_exact_metadata_and_rejects_malformed_descriptors() -> None:
    """Review handles are hash-bound and bounded before they reach the manager."""
    ...
    async def run() -> None:
        ...
        task = asyncio.create_task(
            client.async_prepare_review("meter.yaml", valid["source_sha256"], "proposed")
        )
        await asyncio.sleep(0)
        assert ws.sent[0] == {
            "command": "firmware/prepare_review",
            "message_id": "1",
            "args": {
        ...
        await ws.send_result("1", valid)
        descriptor = await task
        assert descriptor.review_id == "review-1"
        assert descriptor.source_sha256 == "a" * 64
        assert descriptor.proposed_sha256 == "b" * 64
        assert descriptor.inputs_sha256 == "c" * 64
        ...
        for key, value in (
            ...
            client, ws = await connected_client()
            task = asyncio.create_task(
                client.async_prepare_review("meter.yaml", "a" * 64, "proposed")
            )
            await asyncio.sleep(0)
            await ws.send_result("1", {**valid, key: value})
            with pytest.raises(ConnectionError, match="invalid review"):
                await task
            await client.async_disconnect()
...
def test_review_jobs_bind_metadata_and_bound_malformed_artifacts() -> None:
    ...
    async def complete(
        ...
        await asyncio.sleep(0)
        assert ws.sent[0] == {"command": command, "message_id": "1", "args": args}
        await ws.send_result("1", {"job_id": job_id})
        await asyncio.sleep(0)
        await asyncio.sleep(0)
        assert ws.sent[1] == {
        ...
        await ws.send_result("3", full_job)
        return await task

    async def run() -> None:
        client, ws = await connected_client()
        compile_task = asyncio.create_task(
            client.async_compile_review("review-1", "a" * 64, "b" * 64, "c" * 64)
        )
        compile_result = await complete(
            ws,
            compile_task,
            "firmware/compile_review",
            {"review_id": "review-1", "source_sha256": "a" * 64,
             "proposed_sha256": "b" * 64, "inputs_sha256": "c" * 64},
            "compile-job",
            {"job_id": "compile-job", "review_id": "review-1", "inputs_sha256": "c" * 64,
             "artifact_sha256": "d" * 64},
        )
        assert compile_result.success
        assert compile_result.review_id == "review-1"
        assert compile_result.inputs_sha256 == "c" * 64
        assert compile_result.artifact_sha256 == "d" * 64
        await client.async_disconnect()

        client, ws = await connected_client()
        upload_task = asyncio.create_task(
            client.async_upload_review("review-1", "compile-job", "d" * 64)
        )
        upload_result = await complete(
            ws,
            upload_task,
            "firmware/upload_review",
            {"review_id": "review-1", "compile_job_id": "compile-job", "artifact_sha256": "d" * 64},
            "upload-job",
            {"job_id": "upload-job", "review_id": "review-1", "inputs_sha256": "c" * 64,
             "artifact_sha256": "d" * 64},
        )
        assert upload_result.success and upload_result.artifact_sha256 == "d" * 64
        assert ws.sent[0]["args"] == {
            "review_id": "review-1", "compile_job_id": "compile-job", "artifact_sha256": "d" * 64,
        }
        await client.async_disconnect()
        ...
        malformed = await complete(
            ws,
            malformed_task,
            "firmware/upload_review",
            {"review_id": "review-1", "compile_job_id": "compile-job", "artifact_sha256": "d" * 64},
            "upload-job",
            {"job_id": "upload-job", "review_id": "review-1", "artifact_sha256": "not-a-sha"},
        )
        assert malformed.artifact_sha256 is None
        await client.async_disconnect()
        ...
        await asyncio.sleep(0)
        await ws.send_result("1", {"job_id": "compile-job"})
        await asyncio.sleep(0)
        await asyncio.sleep(0)
        await ws.send_event("2", "result", {"status": "completed", "exit_code": 0})
        await asyncio.sleep(0)
        await asyncio.sleep(0)
        await ws.send_result("3", {"job_id": "foreign-job", "review_id": "review-1"})
        with pytest.raises(ConnectionError, match="mismatched job"):
            await mismatch_task
        await client.async_disconnect()
