...
export function summaryStep(topology: MeterTopology | null, session: SessionStatus | null, transaction: TransactionStatus | null,
  ...
  finish: () => void = () => undefined, keepCalibrationInFlash: () => void = () => undefined): TemplateResult {
  ...
  return html`
    <section class="step-content" aria-labelledby="step-heading">
      ${restart && hasOffsets
        ...
        : restart ? html`<div class="success-band" role="status">Setup and exact restart verification are complete. Calibration is stored in meter flash. Installing firmware may replace it.</div>`
      ...
      <footer class="action-footer"><button class="secondary" @click=${back}>Back</button>
        ${handoffAction ? html`${!restart?.source_handoff_firmware_installed ? html`<button class="secondary" data-action="keep-calibration-flash" @click=${keepCalibrationInFlash}>Keep calibration in meter flash</button>` : ""}<button class="primary" data-action="save-calibration" @click=${saveCalibration}>${restart?.source_handoff_firmware_installed ? "Retry clearing saved flash values" : "Save calibration to YAML"}</button>` : ""}
