...
class U extends i {
  ...
  async getChipDescription(E) {
    let S;
    return S = await this.getPkgVersion(E) === 0 ? "ESP32-C61" : "unknown ESP32-C61", `${S} (revision v${await this.getMajorChipVersion(E)}.${await this.getMinorChipVersion(E)})`;
  }
  async getChipFeatures(E) {
    return ["WiFi 6", "BT 5"];
  }
  async readMac(E) {
    let S = await E.readReg(this.MAC_EFUSE_REG);
    S >>>= 0;
    let t = await E.readReg(this.MAC_EFUSE_REG + 4);
    t = t >>> 0 & 65535;
    const _ = new Uint8Array(6);
    return _[0] = t >> 8 & 255, _[1] = 255 & t, _[2] = S >> 24 & 255, _[3] = S >> 16 & 255, _[4] = S >> 8 & 255, _[5] = 255 & S, this._d2h(_[0]) + ":" + this._d2h(_[1]) + ":" + this._d2h(_[2]) + ":" + this._d2h(_[3]) + ":" + this._d2h(_[4]) + ":" + this._d2h(_[5]);
  }
}
export {
  U as ESP32C61ROM
