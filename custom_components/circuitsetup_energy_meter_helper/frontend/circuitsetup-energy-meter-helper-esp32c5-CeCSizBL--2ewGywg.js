import { ESP32C6ROM as i } from "./circuitsetup-energy-meter-helper-esp32c6-DolpfL0e-CfDh6Tsc.js";
...
class A extends i {
  ...
  async getPkgVersion(E) {
    return await E.readReg(this.EFUSE_BLOCK1_ADDR + 8) >> 26 & 7;
  }
  async getMinorChipVersion(E) {
    return await E.readReg(this.EFUSE_BLOCK1_ADDR + 8) >> 0 & 15;
  }
  async getMajorChipVersion(E) {
    return await E.readReg(this.EFUSE_BLOCK1_ADDR + 8) >> 4 & 3;
  }
  async getChipDescription(E) {
    let t;
    return t = await this.getPkgVersion(E) === 0 ? "ESP32-C5" : "unknown ESP32-C5", `${t} (revision v${await this.getMajorChipVersion(E)}.${await this.getMinorChipVersion(E)})`;
  }
  async getChipFeatures(E) {
    return ["Wi-Fi 6 (dual-band)", "BT 5 (LE)", "IEEE802.15.4", "Single Core + LP Core", "240MHz"];
  }
  async getCrystalFreq(E) {
    const t = await E.readReg(this.UART_CLKDIV_REG) & this.UART_CLKDIV_MASK, _ = E.transport.baudrate * t / 1e6 / this.XTAL_CLK_DIVIDER;
    let S;
    return S = _ > 45 ? 48 : _ > 33 ? 40 : 26, Math.abs(S - _) > 1 && E.info("WARNING: Unsupported crystal in use"), S;
  }
  async getCrystalFreqRomExpect(E) {
    return (await E.readReg(this.PCR_SYSCLK_CONF_REG) & this.PCR_SYSCLK_XTAL_FREQ_V) >> this.PCR_SYSCLK_XTAL_FREQ_S;
  }
  async getKeyBlockPurpose(E, t) {
    if (t < 0 || t > this.EFUSE_MAX_KEY) throw new Error(`Valid key block numbers must be in range 0-${this.EFUSE_MAX_KEY}`);
    const _ = [[this.EFUSE_PURPOSE_KEY0_REG, this.EFUSE_PURPOSE_KEY0_SHIFT], [this.EFUSE_PURPOSE_KEY1_REG, this.EFUSE_PURPOSE_KEY1_SHIFT], [this.EFUSE_PURPOSE_KEY2_REG, this.EFUSE_PURPOSE_KEY2_SHIFT], [this.EFUSE_PURPOSE_KEY3_REG, this.EFUSE_PURPOSE_KEY3_SHIFT], [this.EFUSE_PURPOSE_KEY4_REG, this.EFUSE_PURPOSE_KEY4_SHIFT], [this.EFUSE_PURPOSE_KEY5_REG, this.EFUSE_PURPOSE_KEY5_SHIFT]], [S, s] = _[t];
    return await E.readReg(S) >> s & 31;
  }
  async isFlashEncryptionKeyValid(E) {
    const t = [];
    for (let _ = 0; _ <= this.EFUSE_MAX_KEY; _++) {
      const S = await this.getKeyBlockPurpose(E, _);
      t.push(S);
    }
    return t.some(((_) => _ === this.PURPOSE_VAL_XTS_AES128_KEY)) ? !0 : (await E.readReg(this.EFUSE_FORCE_USE_KEY_MANAGER_KEY_REG) >> this.EFUSE_FORCE_USE_KEY_MANAGER_KEY_SHIFT & this.FORCE_USE_KEY_MANAGER_VAL_XTS_AES_KEY) != 0;
  }
  checkSpiConnection(E, t) {
    if (!t.every(((_) => _ >= 0 && _ <= 28))) throw new Error("SPI Pin numbers must be in the range 0-28.");
    t.some(((_) => _ === 13 || _ === 14)) && E.info("GPIO pins 13 and 14 are used by USB-Serial/JTAG, consider using other pins for SPI flash connection.");
  }
  async usesUsbJtagSerial(E) {
    const t = this.UARTDEV_BUF_NO;
    return (255 & await E.readReg(t)) === 3;
  }
  async watchdogReset(E) {
    throw E.info("Hard resetting with a watchdog..."), new Error("watchdogReset not yet implemented for ESP32-C5");
  }
  async changeBaud(E) {
    if (!E.IS_STUB) {
      const t = await this.getCrystalFreqRomExpect(E), _ = await this.getCrystalFreq(E);
      E.info(`ROM expects crystal freq: ${t} MHz, detected ${_} MHz.`), (_ === 48 && t === 40 || _ === 40 && t === 48) && E.info("Crystal frequency mismatch detected. Baud rate adjustment may be needed but is not fully implemented in this version.");
    }
    await E.changeBaud();
  }
}
