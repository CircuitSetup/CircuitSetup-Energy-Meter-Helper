import { R as ts } from "./circuitsetup-energy-meter-helper-install-dialog-im156JnI-CM2eHnjj.js";
import "./circuitsetup-energy-meter-helper-styles-sT2V1cOw-CrAdtexe.js";
let s$1 = class s extends ts {
  constructor() {
    super(...arguments), this.CHIP_NAME = "ESP32", this.IMAGE_CHIP_ID = 0, this.EFUSE_RD_REG_BASE = 1073061888, this.DR_REG_SYSCON_BASE = 1073111040, this.UART_CLKDIV_REG = 1072955412, this.UART_CLKDIV_MASK = 1048575, this.UART_DATE_REG_ADDR = 1610612856, this.XTAL_CLK_DIVIDER = 1, this.IROM_MAP_START = 1074593792, this.IROM_MAP_END = 1077936128, this.DROM_MAP_START = 1061158912, this.DROM_MAP_END = 1065353216, this.MEMORY_MAP = [[0, 65536, "PADDING"], [1061158912, 1065353216, "DROM"], [1065353216, 1069547520, "EXTRAM_DATA"], [1073217536, 1073225728, "RTC_DRAM"], [1073283072, 1073741824, "BYTE_ACCESSIBLE"], [1073405952, 1073741824, "DRAM"], [1073610752, 1073741820, "DIRAM_DRAM"], [1073741824, 1074200576, "IROM"], [1074200576, 1074233344, "CACHE_PRO"], [1074233344, 1074266112, "CACHE_APP"], [1074266112, 1074397184, "IRAM"], [1074397184, 1074528252, "DIRAM_IRAM"], [1074528256, 1074536448, "RTC_IRAM"], [1074593792, 1077936128, "IROM"], [1342177280, 1342185472, "RTC_DATA"]], this.FLASH_SIZES = { "1MB": 0, "2MB": 16, "4MB": 32, "8MB": 48, "16MB": 64, "32MB": 80, "64MB": 96, "128MB": 112 }, this.FLASH_FREQUENCY = { "80m": 15, "40m": 0, "26m": 1, "20m": 2 }, this.FLASH_WRITE_SIZE = 1024, this.BOOTLOADER_FLASH_OFFSET = 4096, this.SPI_REG_BASE = 1072963584, this.SPI_USR_OFFS = 28, this.SPI_USR1_OFFS = 32, this.SPI_USR2_OFFS = 36, this.SPI_W0_OFFS = 128, this.SPI_MOSI_DLEN_OFFS = 40, this.SPI_MISO_DLEN_OFFS = 44;
  }
  async readEfuse(t, s2) {
    const i = this.EFUSE_RD_REG_BASE + 4 * s2;
    return t.debug("Read efuse " + i), await t.readReg(i);
  }
  async getPkgVersion(t) {
    const s2 = await this.readEfuse(t, 3);
    let i = s2 >> 9 & 7;
    return i += (s2 >> 2 & 1) << 3, i;
  }
  async getChipRevision(t) {
    const s2 = await this.readEfuse(t, 3), i = await this.readEfuse(t, 5), e = await t.readReg(this.DR_REG_SYSCON_BASE + 124);
    return 0 != (s2 >> 15 & 1) ? 0 != (i >> 20 & 1) ? 0 != (e >> 31 & 1) ? 3 : 2 : 1 : 0;
  }
  async getChipDescription(t) {
    const s2 = ["ESP32-D0WDQ6", "ESP32-D0WD", "ESP32-D2WD", "", "ESP32-U4WDH", "ESP32-PICO-D4", "ESP32-PICO-V3-02"];
    let i = "";
    const e = await this.getPkgVersion(t), _ = await this.getChipRevision(t), a = 3 == _;
    return 0 != (1 & await this.readEfuse(t, 3)) && (s2[0] = "ESP32-S0WDQ6", s2[1] = "ESP32-S0WD"), a && (s2[5] = "ESP32-PICO-V3"), i = e >= 0 && e <= 6 ? s2[e] : "Unknown ESP32", !a || 0 !== e && 1 !== e || (i += "-V3"), i + " (revision " + _ + ")";
  }
  async getChipFeatures(t) {
    const s2 = ["Wi-Fi"], i = await this.readEfuse(t, 3);
    0 === (2 & i) && s2.push(" BT");
    0 !== (1 & i) ? s2.push(" Single Core") : s2.push(" Dual Core");
    if (0 !== (8192 & i)) {
      0 !== (4096 & i) ? s2.push(" 160MHz") : s2.push(" 240MHz");
    }
    const e = await this.getPkgVersion(t);
    -1 !== [2, 4, 5, 6].indexOf(e) && s2.push(" Embedded Flash"), 6 === e && s2.push(" Embedded PSRAM");
    0 !== (await this.readEfuse(t, 4) >> 8 & 31) && s2.push(" VRef calibration in efuse");
    0 !== (i >> 14 & 1) && s2.push(" BLK3 partially reserved");
    const _ = 3 & await this.readEfuse(t, 6);
    return s2.push(" Coding Scheme " + ["None", "3/4", "Repeat (UNSUPPORTED)", "Invalid"][_]), s2;
  }
  async getCrystalFreq(t) {
    const s2 = await t.readReg(this.UART_CLKDIV_REG) & this.UART_CLKDIV_MASK, i = t.transport.baudrate * s2 / 1e6 / this.XTAL_CLK_DIVIDER;
    let e;
    return e = i > 33 ? 40 : 26, Math.abs(e - i) > 1 && t.info("WARNING: Unsupported crystal in use"), e;
  }
  _d2h(t) {
    const s2 = (+t).toString(16);
    return 1 === s2.length ? "0" + s2 : s2;
  }
  async readMac(t) {
    let s2 = await this.readEfuse(t, 1);
    s2 >>>= 0;
    let i = await this.readEfuse(t, 2);
    i >>>= 0;
    const e = new Uint8Array(6);
    return e[0] = i >> 8 & 255, e[1] = 255 & i, e[2] = s2 >> 24 & 255, e[3] = s2 >> 16 & 255, e[4] = s2 >> 8 & 255, e[5] = 255 & s2, this._d2h(e[0]) + ":" + this._d2h(e[1]) + ":" + this._d2h(e[2]) + ":" + this._d2h(e[3]) + ":" + this._d2h(e[4]) + ":" + this._d2h(e[5]);
  }
};
export {
  s$1 as ESP32ROM
};
