import { ESP32ROM as i } from "./circuitsetup-energy-meter-helper-esp32-DNPRK0Ay-BmbbHw2T.js";
...
class a extends i {
  ...
  async getChipDescription(_) {
    const E = await this.getFlashCap(_) + 100 * await this.getPsramCap(_), s = await this.getMajorChipVersion(_), t = await this.getMinorChipVersion(_);
    return `${{ 0: "ESP32-S2", 1: "ESP32-S2FH2", 2: "ESP32-S2FH4", 102: "ESP32-S2FNR2", 100: "ESP32-S2R2" }[E] || "unknown ESP32-S2"} (revision v${s}.${t})`;
  }
  async getFlashCap(_) {
  ...
  async getPsramVersion(_) {
    ...
    return await _.readReg(E) >> 28 & 15;
  }
  async getPsramCap(_) {
  ...
  async getBlock2Version(_) {
    ...
    return await _.readReg(E) >> 4 & 7;
  }
  async getChipFeatures(_) {
    const E = ["Wi-Fi"], s = { 0: "No Embedded Flash", 1: "Embedded Flash 2MB", 2: "Embedded Flash 4MB" }[await this.getFlashCap(_)] || "Unknown Embedded Flash";
    E.push(s);
    const t = { 0: "No Embedded Flash", 1: "Embedded PSRAM 2MB", 2: "Embedded PSRAM 4MB" }[await this.getPsramCap(_)] || "Unknown Embedded PSRAM";
    E.push(t);
  ...
  _d2h(_) {
    const E = (+_).toString(16);
    return E.length === 1 ? "0" + E : E;
  }
  async readMac(_) {
    let E = await _.readReg(this.MAC_EFUSE_REG);
    E >>>= 0;
    let s = await _.readReg(this.MAC_EFUSE_REG + 4);
    s = s >>> 0 & 65535;
    const t = new Uint8Array(6);
    return t[0] = s >> 8 & 255, t[1] = 255 & s, t[2] = E >> 24 & 255, t[3] = E >> 16 & 255, t[4] = E >> 8 & 255, t[5] = 255 & E, this._d2h(t[0]) + ":" + this._d2h(t[1]) + ":" + this._d2h(t[2]) + ":" + this._d2h(t[3]) + ":" + this._d2h(t[4]) + ":" + this._d2h(t[5]);
  }
  getEraseSize(_, E) {
    return E;
  }
  ...
  async postConnect(_) {
    const E = await this.usingUsbOtg(_);
    _.debug("In _post_connect using USB OTG ?" + E), E && (_.ESP_RAM_BLOCK = this.USB_RAM_BLOCK);
  }
}
export {
  a as ESP32S2ROM
