...
class _ extends s {
  ...
  async getChipDescription(E) {
    let _2;
    _2 = 0 === await this.getPkgVersion(E) ? "ESP32-C5" : "unknown ESP32-C5";
    return `${_2} (revision v${await this.getMajorChipVersion(E)}.${await this.getMinorChipVersion(E)})`;
  }
  async getChipFeatures(E) {
    return ["Wi-Fi 6 (dual-band)", "BT 5 (LE)", "IEEE802.15.4", "Single Core + LP Core", "240MHz"];
  }
  async getCrystalFreq(E) {
    const _2 = await E.readReg(this.UART_CLKDIV_REG) & this.UART_CLKDIV_MASK, t = E.transport.baudrate * _2 / 1e6 / this.XTAL_CLK_DIVIDER;
    let S;
    return S = t > 45 ? 48 : t > 33 ? 40 : 26, Math.abs(S - t) > 1 && E.info("WARNING: Unsupported crystal in use"), S;
  }
  ...
  async getKeyBlockPurpose(E, _2) {
    if (_2 < 0 || _2 > this.EFUSE_MAX_KEY) throw new Error(`Valid key block numbers must be in range 0-${this.EFUSE_MAX_KEY}`);
    const t = [[this.EFUSE_PURPOSE_KEY0_REG, this.EFUSE_PURPOSE_KEY0_SHIFT], [this.EFUSE_PURPOSE_KEY1_REG, this.EFUSE_PURPOSE_KEY1_SHIFT], [this.EFUSE_PURPOSE_KEY2_REG, this.EFUSE_PURPOSE_KEY2_SHIFT], [this.EFUSE_PURPOSE_KEY3_REG, this.EFUSE_PURPOSE_KEY3_SHIFT], [this.EFUSE_PURPOSE_KEY4_REG, this.EFUSE_PURPOSE_KEY4_SHIFT], [this.EFUSE_PURPOSE_KEY5_REG, this.EFUSE_PURPOSE_KEY5_SHIFT]], [S, s2] = t[_2];
    return await E.readReg(S) >> s2 & 31;
  }
  async isFlashEncryptionKeyValid(E) {
    const _2 = [];
    for (let t = 0; t <= this.EFUSE_MAX_KEY; t++) {
      const S = await this.getKeyBlockPurpose(E, t);
      _2.push(S);
    }
    if (_2.some(((E2) => E2 === this.PURPOSE_VAL_XTS_AES128_KEY))) return true;
    return 0 != (await E.readReg(this.EFUSE_FORCE_USE_KEY_MANAGER_KEY_REG) >> this.EFUSE_FORCE_USE_KEY_MANAGER_KEY_SHIFT & this.FORCE_USE_KEY_MANAGER_VAL_XTS_AES_KEY);
  }
  checkSpiConnection(E, _2) {
    if (!_2.every(((E2) => E2 >= 0 && E2 <= 28))) throw new Error("SPI Pin numbers must be in the range 0-28.");
    _2.some(((E2) => 13 === E2 || 14 === E2)) && E.info("GPIO pins 13 and 14 are used by USB-Serial/JTAG, consider using other pins for SPI flash connection.");
  }
  async usesUsbJtagSerial(E) {
    const _2 = this.UARTDEV_BUF_NO;
    return 3 === (255 & await E.readReg(_2));
  }
  ...
  async changeBaud(E) {
    if (!E.IS_STUB) {
      const _2 = await this.getCrystalFreqRomExpect(E), t = await this.getCrystalFreq(E);
      E.info(`ROM expects crystal freq: ${_2} MHz, detected ${t} MHz.`), (48 === t && 40 === _2 || 40 === t && 48 === _2) && E.info("Crystal frequency mismatch detected. Baud rate adjustment may be needed but is not fully implemented in this version.");
    }
    await E.changeBaud();
  }
}
