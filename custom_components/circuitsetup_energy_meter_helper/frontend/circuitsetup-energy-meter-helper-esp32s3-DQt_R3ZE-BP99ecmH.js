import { ESP32ROM as S } from "./circuitsetup-energy-meter-helper-esp32-DNPRK0Ay-BmbbHw2T.js";
...
class F extends S {
  ...
  async getChipDescription(t) {
    const i = await this.getMajorChipVersion(t), e = await this.getMinorChipVersion(t);
    return `${{ 0: "ESP32-S3 (QFN56)", 1: "ESP32-S3-PICO-1 (LGA56)" }[await this.getPkgVersion(t)] || "unknown ESP32-S3"} (revision v${i}.${e})`;
  }
  ...
  async isEco0(t, i) {
    return (7 & i) == 0 && await this.getBlkVersionMajor(t) === 1 && await this.getBlkVersionMinor(t) === 1;
  }
  async getFlashCap(t) {
  ...
  async getFlashVendor(t) {
    ...
    return { 1: "XMC", 2: "GD", 3: "FM", 4: "TT", 5: "BY" }[await t.readReg(i) >> 0 & 7] || "";
  }
  async getPsramCap(t) {
    const i = this.EFUSE_BASE + 68 + 16;
  ...
  async getChipFeatures(t) {
    const i = ["Wi-Fi", "BLE"], e = await this.getFlashCap(t), s = await this.getFlashVendor(t), a = { 0: null, 1: "Embedded Flash 8MB", 2: "Embedded Flash 4MB" }[e], n = a !== void 0 ? a : "Unknown Embedded Flash";
    a !== null && i.push(`${n} (${s})`);
    const _ = await this.getPsramCap(t), h = await this.getPsramVendor(t), r = { 0: null, 1: "Embedded PSRAM 8MB", 2: "Embedded PSRAM 2MB" }[_], E = r !== void 0 ? r : "Unknown Embedded PSRAM";
    return r !== null && i.push(`${E} (${h})`), i;
  }
  ...
  async postConnect(t) {
    ...
    t.debug("In _post_connect " + i), i == this.UARTDEV_BUF_NO_USB && (t.ESP_RAM_BLOCK = this.USB_RAM_BLOCK);
  }
  async readMac(t) {
    ...
    return s[0] = e >> 8 & 255, s[1] = 255 & e, s[2] = i >> 24 & 255, s[3] = i >> 16 & 255, s[4] = i >> 8 & 255, s[5] = 255 & i, this._d2h(s[0]) + ":" + this._d2h(s[1]) + ":" + this._d2h(s[2]) + ":" + this._d2h(s[3]) + ":" + this._d2h(s[4]) + ":" + this._d2h(s[5]);
  }
  getEraseSize(t, i) {
    return i;
  }
