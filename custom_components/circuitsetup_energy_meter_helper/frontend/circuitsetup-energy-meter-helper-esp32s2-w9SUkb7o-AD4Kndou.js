...
class E extends s$1 {
  ...
  async readMac(_) {
    ...
    return s[0] = t >> 8 & 255, s[1] = 255 & t, s[2] = E2 >> 24 & 255, s[3] = E2 >> 16 & 255, s[4] = E2 >> 8 & 255, s[5] = 255 & E2, this._d2h(s[0]) + ":" + this._d2h(s[1]) + ":" + this._d2h(s[2]) + ":" + this._d2h(s[3]) + ":" + this._d2h(s[4]) + ":" + this._d2h(s[5]);
  }
  getEraseSize(_, E2) {
    return E2;
  }
  async usingUsbOtg(_) {
    return (255 & await _.readReg(this.UARTDEV_BUF_NO)) === this.UARTDEV_BUF_NO_USB_OTG;
  }
  async postConnect(_) {
    const E2 = await this.usingUsbOtg(_);
    _.debug("In _post_connect using USB OTG ?" + E2), E2 && (_.ESP_RAM_BLOCK = this.USB_RAM_BLOCK);
  }
}
export {
  E as ESP32S2ROM
